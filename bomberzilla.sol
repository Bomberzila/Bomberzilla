...
contract Bomberzilla is Context, Ownable, ERC20 {
    using Address for address;

    IFeeHandler public feeHandler;
    uint256 public constant MAX_TRANSFER_FEE = 1500; // 15%

    event FeeHandlerUpdated(
        address indexed oldFeeHandler,
        address indexed newFeeHandler
    );

    constructor() ERC20("Bomberzilla", "BBZ") {
    ...
    function _transfer(
        ...
        if (address(feeHandler).isContract())
            try feeHandler.getFeeInfo(sender, recipient, amount) returns (
                uint256 fee
            ) {
                if (fee > 0 && fee <= MAX_TRANSFER_FEE) {
                    ...
                    try
                        feeHandler.onFeeReceived(sender, recipient, amount, fee)
                    {} catch {}
                }
            } catch {}
